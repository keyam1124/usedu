...
max entries や max duration などの traversal budget も、`RESOURCE_LIMIT_REACHED` を持つ partial scan issue として表現します。
serialized response の max entries や max bytes のような output cap は `limitReached` を返しますが、cleanup safety を意味しません。
