...
この文書は B01 の実装計画を記録します。
現時点では runtime behavior を変更しません。
...
migration を実装するまでは、現在の `--json` output を current JSON format として維持します。
既存 format をその場で変更せず、明示的な machine format を追加します。
...
将来、現在の `--json` flag は current format の documented alias にするか、stderr に migration warning を出す形にできます。
